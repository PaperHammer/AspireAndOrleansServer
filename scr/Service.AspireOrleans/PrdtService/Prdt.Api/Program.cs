    ...
    public static void Main(string[] args)
    {
        ...
        builder.UseOrleans(orleansBuilder =>
        {
            if (builder.Environment.IsDevelopment())
            {
                ...
                orleansBuilder.Services.AddSingleton<DbContext, ProductDbContext>();
        ...
        builder.Services.AddDbContextPool<ProductDbContext>(options =>
            options.UseSqlServer(builder.Configuration.GetValue<string>("SqlConnectionStrings"), sqlOptions =>
            {
                ...
                sqlOptions.MigrationsAssembly("Prdt.Repository");
                // Workround for https://github.com/dotnet/aspire/issues/1023
                sqlOptions.ExecutionStrategy(c => new RetryingSqlServerRetryingExecutionStrategy(c));
            })
        );
        builder.EnrichSqlServerDbContext<ProductDbContext>(settings =>
