...
namespace Infra.Core.Core.Guard
{
    public class Checker
    {
        ...
        public class Argument
        {
            ...
            public static void IsIllegalEmail(string email)
            {
                Regex regex = new("^\\w+([-+.]\\w+)*@\\w+([-.]\\w+)*\\.\\w+([-.]\\w+)*$"); // 在编译时生成正则表达式

                if (!regex.Match(email).Success)
                {
                    throw new ArgumentException(string.Format($"\"{email}\" is not Illegal email account"));
                }
            }

            public static void IsEqual(string arg1, string arg2, string arg1Name, string arg2Name)
            {
                if (arg1 != arg2)
                {
                    throw new ArgumentException(string.Format($"\"{arg1Name}\" and \"{arg2Name}\" is not euqal."));
                }
            }
