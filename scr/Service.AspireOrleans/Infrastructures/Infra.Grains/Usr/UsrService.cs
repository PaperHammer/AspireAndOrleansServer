...
namespace Grains.Usr
{
    public class UsrService(
            UserDbContext userDbContext,
            ILogger<UsrService> logger)
            : Grain, IUsrService
    {
        ...
        public async Task<AppSrvResult<UserValidatedInfoDto>> LoginAsync(UserLoginDto input)
        {
            try
            {
                ...
                var user = await _userDbContext.Users.FirstOrDefaultAsync(
                    u => u.Email == input.Email && u.Password == encryPwd);
                if (user == null)
        ...
        public async Task<AppSrvResult<HttpStatusCode>> UpdateAccountRolesAsync(long id, RolesEnum type)
        {
            try
            {
                var user = await _userDbContext.Users.FirstOrDefaultAsync(u => u.Uid == id);
